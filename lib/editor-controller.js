...
export default class EditorController {
  constructor(editor) {
    ...
    // config
    this.subscriptions.add(atom.config.onDidChange('editor.fontFamily', () => {
      this.clearCursorCaches();
    }));
  ...
  addPreviousLine() {
    ...
    for (const cursor of cursorCtrlers.keys()) {
      const pos = posInfo.get(cursor).newScreenPos;
      if (this.editor.getCursorAtScreenPosition(pos) === undefined) {
        this.editor.addCursorAtScreenPosition(pos);
      }
    }
  }
